...
protected:
  void SetUp() override {}
  void parse_success(const char *file_name,
...
TEST_F(NginxConfigParserTest, QuotesConfig) {
  parse_success("configs/quotes_config",
                "port '';\nport \"\";\nport '\\'';\nport \"\\\"\";\nlocation "
...
TEST_F(NginxConfigParserTest, Comments) {
  parse_success("configs/comments_config", "port 80;\n");
}
...
protected:
  ...
  void find_port_failure() { EXPECT_EQ(full_parsed_config.findPort(), -1); }
  void find_locations_success(
      std::unordered_map<std::string, LocationData> expected_locations) {
    std::optional<std::unordered_map<std::string, LocationData>> locations =
        full_parsed_config.findLocations();
    EXPECT_TRUE(locations.has_value());
    EXPECT_EQ(locations.value().size(),
              expected_locations.size()); // same number of locations
    for (auto loc : locations.value()) {
      EXPECT_TRUE(expected_locations.contains(
          loc.first)); // path exists in expected_locations
      LocationData location_to_test =
          expected_locations[loc.first]; // expected location with same path
      EXPECT_EQ(location_to_test.handler_,
                loc.second.handler_); // handlers are the same
      EXPECT_EQ(location_to_test.arg_map_.size(),
                loc.second.arg_map_.size()); // same number of args
      for (auto arg : loc.second.arg_map_) {
        EXPECT_TRUE(location_to_test.arg_map_.contains(
            arg.first)); // arg exists in expected
        EXPECT_EQ(location_to_test.arg_map_[arg.first],
                  arg.second); // passed in value to arg matches expected
      }
    }
  }
...
// trailing slashes on locations should be "ignored" for the purposes of the
...
// config with static handler taking more than one root should parse
// successfully, but finding locations should fail
