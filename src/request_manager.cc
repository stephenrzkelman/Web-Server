...
http_response RequestManager::manageRequest(boost::asio::mutable_buffer request){
  ...
  if(isGetRequest(parsed_request)){
    ...
    std::optional<std::string> longest_matched_prefix_optional = matchPath(target_path);

    std::shared_ptr<RequestHandler> handler;
    if(!longest_matched_prefix_optional.has_value()){
      BOOST_LOG_TRIVIAL(info) << "No matching path/handler found, something wrong on our end, returning 404 error";
